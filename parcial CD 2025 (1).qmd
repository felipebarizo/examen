...
```{r}
receta <- recipe(Class~., data = train) |> step_impute_median(all_numeric_predictors()) |>step_normalize(all_numeric_predictors())
```
...
pred |> metricas(truth = Class, estimate = .pred_class)
...
```{r}
conf_mat(pred, truth = Class, estimate = .pred_class) |> autoplot(type = "heatmap")
```
...
pred1 <- predict(ajuste_arbol, test) |> bind_cols(test)
```
