...
library(dplyr)
library(tidyverse)
tic <- read_xlsx(here("Rev2024","eutic (2).xlsx"))
...
```{r}
tic_red |> filter(!is.na(C9)) |> mutate(C9_nueva = case_when(C9 ==  2 ~ "No", C9 == 1 ~ "Si")) |> ggplot(aes(x= C9_nueva, fill = C9_nueva)) + 
  geom_bar() + labs( x = "Usa Internet",  y = "Cantidad de personas", fill = "Respuesta")
...
tic_reco <- tic_red |> rename("usoint" = C9  , "sexo" = C7) |> mutate(usoint = as.factor(usoint), sexo  = as.factor(sexo)) |> 
  mutate(usoint = recode(usoint, "2" ="No","1" = "Si"),sexo=  recode(sexo,"1" = "Hombre","2" = "Mujer" ))
...
tic_reco |> count(usoint) |> mutate(prop  = n/sum(n)) |> ggplot(aes(x = usoint, y = prop, fill= usoint)) + geom_col() +theme_bw()+labs( x = "Usa Internet",  y = "Proporcio de Uso de internet", fill = "Respuesta")
...
```{r}
tic_reco |> select(usoint,sexo)|> count(sexo, usoint) |>group_by(sexo)|> mutate(prop = n / sum(n)) |> filter(sexo == "Mujer" & usoint == "Si")
...
tic_reco |>filter(!is.na(C14_1:C14_18))|> mutate(across(C14_1:C14_18, ~recode(.x,"1"= "Si", "2" = "No", "3" = "No Contesta")))
...
"La investigación reproducible es aquella en la que cualquier persona puede volver a ejecutar tu análisis y obtener los mismos resultados, porque tiene acceso a los datos, el código y los pasos exactos que seguiste.

No se trata solo de compartir los resultados finales, sino de documentar todo el proceso, desde la limpieza de datos hasta los gráficos y modelos utilizados.
RMARKDOWN"
```
**4.6** (2.5 Puntos) 
